...
\begin{figure}[htbp]
    ...
    \caption{Pricing \& Plans Interface: Comparison of subscription tiers (Free, Pro) denominated in Egyptian Pounds (EGP) with monthly/annual billing option.}
    \label{fig:app_ui_pricing}
\end{figure}

Key features of the pricing interface:
\begin{itemize}
    \item \textbf{Two-tier pricing structure:} Free (EGP 0/month) and Pro (EGP 23/month).
    \item \textbf{Usage limits:} Free plan provides up to 1,000 renders and 1,000 3D models. Pro plan provides unlimited renders and 3D models.
    \item \textbf{Billing interval toggle:} Supports monthly and annual billing options with a highlighted 17\% savings badge for annual billing.
    \item \textbf{Clear calls-to-action:} Status indicator for the active subscription (``Current Plan'') and clear purchase action button (``Choose Pro'').
\end{itemize}

This interface satisfies requirement FR-27 (subscription purchase via Fawaterk).
