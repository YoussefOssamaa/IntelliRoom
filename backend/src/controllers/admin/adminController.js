...
// import {adminAuthPrivateKey} from '../../utils/getKeys.js';
...
export const getAdminLogs = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error while fetching logs" });
  }
};
...
export const signIn = async (req, res) => {
  try {
    ...
    const admin = await Admin.findOne({ email }).select(
      "+password +failedLoginAttempts +lockUntil +firstFailedAttempt",
    );
    ...
    if (!isMatch) {
      ...
      if (
        !admin.firstFailedAttempt ||
        NOW - new Date(admin.firstFailedAttempt).getTime() > TWENTY_MINS
      ) {
      ...
      if (admin.failedLoginAttempts >= 3) {
        ...
        return res.status(403).json({
          success: false,
          message:
            "Account locked for 1 hour due to 3 consecutive failed login attempts.",
        });
      }
...
export const Refresh = async (req, res) => {
  try {
    ...
    try {
      payload = jwt.verify(refreshToken, adminRefreshPublicKey, {
        algorithms: ["RS256"],
      });
    } catch (err) {
      return res
...
export const getAllAdmins = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error while fetching admins" });
  }
};
...
export const unlockAdmin = async (req, res) => {
  try {
    ...
    const admin = await Admin.findById(id).select(
      "+failedLoginAttempts +lockUntil +firstFailedAttempt",
    );
    ...
    if (!admin.isLocked() && admin.failedLoginAttempts === 0) {
      return res.status(400).json({
        success: false,
        message: "This account is already active and not locked",
      });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error while unlocking admin" });
  }
};
