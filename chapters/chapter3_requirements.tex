...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|p{3.5cm}|p{1.5cm}|p{1.8cm}|p{1.8cm}|p{1.5cm}|X|}
        ...
        Tutorial & 1 & 0 & 0 & 1 & 0\% \\
        \hline
        \textbf{Overall} & \textbf{38} & \textbf{31} & & \textbf{5} & \textbf{82\%} \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{AI Vector Recommendation Engine:} Two-plane architecture with a local FastAPI orchestrator (ChromaDB vector store, semantic taxonomy snapper) and a remote GPU inference server (RT-DETR for detection, SigLIP2 for embeddings, Qwen3-VL 4B for taxonomy tagging) connected via zrok tunneling
