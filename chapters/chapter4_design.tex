...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{AI Recommendation Service:} A Python-based FastAPI orchestrator running locally, connected to a remote GPU inference server via zrok tunneling. Uses RT-DETR for detection, SigLIP2 for embeddings, Qwen3-VL for taxonomy tagging, and ChromaDB for vector storage and retrieval.
    \end{itemize}
...
\begin{figure}[htbp]
    ...
    \caption{Two-Plane Architecture: Local control plane (FastAPI orchestrator, ChromaDB, Semantic Snapper) communicates with the remote GPU inference plane (RT-DETR, SigLIP2, Qwen3-VL) via a zrok public tunnel.}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{RT-DETR r50vd:} Real-time Detection Transformer for furniture bounding box extraction and crop generation.
...
\begin{enumerate}
    ...
    \item \textbf{Detection:} Each image is sent to the remote \texttt{/detect} endpoint. RT-DETR extracts bounding boxes for detected furniture items with confidence scores.
...
\begin{enumerate}
    \item \textbf{Detection:} The room photo is sent to \texttt{/detect}. RT-DETR returns all detected furniture crops with labels, confidence scores, and bounding boxes.
...
The remote GPU backend loads PyTorch, HuggingFace Transformers, and model-specific processors (RT-DETR, SigLIP2, Qwen3-VL) at server startup. Models are cached in persistent storage to avoid re-downloading across sessions.
...
To ensure strict tenant and context segregation, the frontend implements a subdomain-based routing split in \texttt{Routes.jsx}. The application dynamically inspects the current hostname (\texttt{window.location.hostname}) to isolate administrative panels from user-facing screens:
\begin{itemize}
    \item \textbf{Consumer Context:} The root domain serves consumer-facing paths, including the 2D/3D design planner, style uploads, product discovery, and cart/checkout systems.
    \item \textbf{Administrative Context:} Subdomains starting with \texttt{admin} (e.g., \texttt{admin.localhost}) mount a separate router root. This root exposes dashboards for managing user accounts, audit logs, system configurations, and pricing plans. Access is enforced by the \texttt{ProtectedAdminRoute} wrapper.
\end{itemize}
This client-side split prevents normal users from traversing administrative routes. To prevent cross-context session contamination, the backend separates session states into distinct cookie namespaces: \texttt{Authentication}/\texttt{Refresh} for consumers and \texttt{Admin-Authentication}/\texttt{Admin-Refresh} for administrators.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|p{3.5cm}|p{2.8cm}|p{2.2cm}|X|}
        ...
        \hline
        Object Detection & RT-DETR r50vd & Done & Real-time furniture crop extraction \\
        \hline
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|p{4cm}|p{2cm}|X|}
        ...
        \hline
        RT-DETR Detection & Complete & Furniture crop extraction (remote GPU backend) \\
        \hline
