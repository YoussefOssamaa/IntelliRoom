        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-4 w-full">
          ...
          <div className="flex items-center gap-4 lg:gap-6 shrink-0">
            ...
            <nav className="hidden md:flex items-center space-x-4 border-l border-gray-200 pl-4 shrink-0">
              <Link to="/ecomm" className={getLinkClass("/")}>Home</Link>
          ...
          <div className="flex items-center justify-end gap-5 shrink-0 mt-4 lg:mt-0">
            ...
            {/* 1. Favorites Icon */}
            <Link to="/ecomm/wishlist" className="relative text-gray-600 hover:text-red-500 transition-colors p-1">
            ...
            {/* 2. Cart Icon */}
            <Link to="/ecomm/cart" className="relative text-gray-600 hover:text-blue-500 transition-colors p-1 group">
              <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {user ? (
                ...
                {showProfileMenu && (
                  <div className="absolute right-0 mt-2 w-56 bg-white border border-[#e0e0e0] rounded-xl shadow-lg z-50 overflow-hidden">
                    <div className="px-4 py-3 border-b border-[#e0e0e0] bg-gray-50">
                      <p className="text-sm font-semibold text-[#333333]">{user?.user_name || "User"}</p>
                      <p className="text-xs text-[#666666] truncate">{user?.plan || "free"} Plan</p>
                    </div>
                    <ul className="py-2">
                      <li><Link to="/dashboard" className="block px-4 py-2 text-sm text-[#333333] hover:bg-gray-50 transition-colors">Dashboard</Link></li>
                      <li><button className="w-full text-left px-4 py-2 text-sm text-[#333333] hover:bg-gray-50 transition-colors">Settings</button></li>
                      <li className="border-t border-[#e0e0e0] mt-2 pt-2">
                        <button onClick={handleLogout} className="w-full px-4 py-2 text-left text-sm font-bold text-red-600 hover:bg-red-50 transition-colors">
                          Sign Out
                        </button>
                      </li>
                    </ul>
              ...
              <div className="flex items-center gap-3 pl-2 border-l border-gray-200">
                <Link 
                  to="/login" 
                  // className="px-3 py-2 text-sm font-bold text-text-primary hover:text-text-accent transition-colors"
                ...
                <Link 
                  to="/signup" 
                  // className="px-4 py-2 bg-text-accent text-white text-sm font-bold rounded-lg hover:opacity-90 transition-opacity shadow-sm whitespace-nowrap"
