...
const AdminProductEditPage = () => {
  // Grab the parameter from the URL (works whether your Route is set to :id or :slug)
  ...
  // 🚀 NEW: We need to hold onto the real database _id for our PUT request!
  ...
  useEffect(() => {
    const fetchProductAndOptions = async () => {
      try {
        // 🚀 THE FIX: Fetch using the slug (identifier) just like the customer details page!
        const [optionsRes, productRes] = await Promise.all([
          axios.get("/products/form-options"),
          axios.get(`/products/${identifier}`),
        ]);

        // 1. Process Form Options
        if (optionsRes.data.success) {
          const { colors, materials, rooms, categories } = optionsRes.data.data;
        ...
        // 2. Process & Pre-fill Existing Product Data
        if (productRes.data.success) {
          const p = productRes.data.data;

          // 🚀 Save the real _id so we can update it later
          setProductId(p._id);
          ...
          setFormData({
            ...
            currentPrice: p.pricing?.currentPrice || "",
            originalPrice: p.pricing?.originalPrice || "",
            costPerItem: p.pricing?.costPerItem || "",
            stockQuantity: p.inventory?.stockQuantity || "",

            width: p.physical?.dimensions?.width || "",
            height: p.physical?.dimensions?.height || "",
            depth: p.physical?.dimensions?.depth || "",
            weight: p.physical?.weight || "",
            assemblyRequired: p.physical?.assemblyRequired || false,

            shippingWidth: p.inventory?.shippingDimensions?.width || "",
            shippingHeight: p.inventory?.shippingDimensions?.height || "",
            shippingDepth: p.inventory?.shippingDimensions?.depth || "",
            shippingWeight: p.inventory?.shippingDimensions?.weight || "",

            primaryImage: p.media?.primaryImage || "",
            gallery: p.media?.gallery ? p.media.gallery.join(", ") : "",
            threeDModelUrl: p.media?.threeDModelUrl || "",

            primaryCategory:
              p.categorization?.primary?.name ||
              p.categorization?.primary ||
              "",
            subCategory:
              p.categorization?.subCategory?.name ||
              p.categorization?.subCategory ||
              "",
            colors: p.categorization?.colors || [],
            materials: p.categorization?.materials || [],
            rooms: p.categorization?.rooms?.map((r) => r._id || r) || [],
            tags: p.categorization?.tags
              ? p.categorization.tags.join(", ")
              : "",
          });
        }
      } catch (err) {
        console.error("Failed to load product or options:", err);
        setError(
          "Could not load product data from the server. Check if the URL slug is correct.",
        );
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 🚀 THE FIX: Use the saved real database _id for the PUT request
      const response = await axios.put(`/products/${productId}`, payload);
      if (response.data.success || response.status === 200) {
        navigate("/ecomm/admin/products/list");
      }
    } catch (err) {
      console.error("Update error:", err);
      setError(
        err.response?.data?.message ||
          "Failed to update product. Check server logs.",
      );
      window.scrollTo({ top: 0, behavior: "smooth" });
    } finally {
      setIsLoading(false);
