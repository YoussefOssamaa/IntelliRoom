...
import _paintedJpg from './painted.jpg';
import _bricksJpg from './bricks.jpg';
import _balconyPng from './balcony.png';

const PI_2 = Math.PI / 2;
const paintedtexture = _paintedJpg;
const brickTexture = _bricksJpg;
const scale = 100;
...
function makeObject(newWidth, newHeight, newDepth) {
  ...
  // Create materials first without textures
  let cubeMaterial = new Three.MeshLambertMaterial({ color: 0xffffff, map: null });
  let cubeMaterial2 = new Three.MeshLambertMaterial({ color: 0xffffff, map: null });
  let cubeMaterial3 = new Three.MeshLambertMaterial({ color: 0xffffff, map: null });

  // Load textures and assign to materials in callbacks
  textureLoader.load(paintedtexture, (texture) => {
    cubeMaterial.map = texture;
    cubeMaterial.needsUpdate = true;
  });

  textureLoader.load(brickTexture, (texture) => {
  ...
  textureLoader.load(brickTexture, (texture) => {
    ...
    cubeMaterial3.needsUpdate = true;
  });
