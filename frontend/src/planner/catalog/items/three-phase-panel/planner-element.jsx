...
import _panelTexturePng from './PanelTexture.png';
import _threePhasePanelPng from './threePhasePanel.png';
...
let quadroMaterial = new Three.MeshPhongMaterial({ transparent: true, color: 0xffffff, map: null });

// Load texture and assign in callback
textureLoader.load(_panelTexturePng, (texture) => {
  quadroMaterial.map = texture;
  quadroMaterial.needsUpdate = true;
});
...
function makeObjectMaxLOD() {
  ...
  let meshPanel = new Three.Mesh( PanelGeometry, quadroMaterial.clone() );
...
function makeObjectMiddleLOD() {
  ...
  let PanelGeometry = new Three.PlaneGeometry(0.5,0.5);
  let Panel = new Three.Mesh( PanelGeometry, quadroMaterial.clone() );
